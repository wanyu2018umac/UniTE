...
class XLMREncoder(BERTEncoder):
    ...
    def freeze_embeddings(self) -> None:
        ...
        print()
        print('Freeze embedding function from xlmr.py')
        # input()
        for name, param in self.model.embeddings.named_parameters():
            if 'token_type_embeddings' in name and param.size(0) > 1:
                param.requires_grad = True
                print('Unfreeze position embeddings %s for RoBERTa model, size: %s.' % (name, param.size()))
                continue
            else:
                print('Freeze %s, size: %s' % (name, param.size()))                
    ...
    def forward(
        self, input_ids: torch.Tensor, attention_mask: torch.Tensor, token_type_ids: torch.Tensor, position_ids: torch.Tensor, **kwargs
